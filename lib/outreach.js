...
// A warm, ready-to-personalize reply for when a business writes back.
// Never auto-sent — a real reply means a human is engaged, so this is a
// starter Nick edits and sends himself.
export function renderReplySuggestion(p) {
  return [
    `Hi ${p.greeting_name || "there"},`,
    ``,
    `Really glad you're interested - thanks for getting back to me.`,
    ``,
    `Here's all it takes to get ${p.business} on the board:`,
    `1. We lock in a first offer - I'd suggest ${p.offer_idea || "a comped visit for a 30-second video"}.`,
    `2. I post it at bagdit.app/offers and a local creator claims it.`,
    `3. They come in as a normal paying customer, film 30 seconds, and you approve the video before you owe anything.`,
    ``,
    `Want to hop on a quick call, or should I just send the one-page setup? Either's easy.`,
    ``,
    `Nick`,
  ].join("\n");
}
